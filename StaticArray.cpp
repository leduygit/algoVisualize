...
StaticArray::StaticArray(sf::Color textColor, sf::Font &font, sf::Vector2f buttonSize, sf::Color backgroundColor)
{
    ...
    {
        nodes[i] = ArrNode("?", textColor, font, 70.f, backgroundColor);
...
void StaticArray::mainDraw(sf::RenderWindow &window, sf::Sprite &background)
{
    ...
    for (int i = 0; i < SASize; ++i)
    {
        ...
        if (i + 1 > 9) index.setPosition({position.x + 20, position.y + 75});
        if (i + 1 == usedSize) index.setFillColor(sf::Color::Red);
        else index.setFillColor(sf::Color::Black);
        window.draw(index);
        if (i + 1 == usedSize)
        {
...
void StaticArray::accessPosAnimation(sf::RenderWindow &window, sf::Sprite &background)
{
}
